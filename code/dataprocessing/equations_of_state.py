...
def eos_water_viscosity_1(T, P, PS):
    ...
    EX  = 247.8 / (T+133.15)
    PHI = 1.0467 * (T-31.85)
    AM  = 1.0 + PHI*(torch.max(P,PS) - PS) * 1.0e-11
    pwr = torch.pow(10, EX)
...
#-----data aa20,aa21,aa22/
    ...
    yy = one-a1*theta2x-a2*torch.pow(theta, -6.0)
    xx = a3*yy*yy-two*(a4*theta-a5*beta)
    xx = torch.sqrt(xx)

    zz = yy + xx
    u0 = -five/17.0
    u1 = aa[11]*a5*torch.pow(zz, u0)

    ypt = six*a2*torch.pow(theta, -7.0)-two*a1*theta
    ...
    #do i = 3,10
    for i in range(3,11):
        tempreal = (i-2)*aa[i]*torch.pow(theta, i-1)
    ...
    v1_2 = nine*theta+a6
    v20_2 = (a6-theta)
    v2_2 = torch.pow(v20_2,9.0)
    v3_2 = a7+20.0*torch.pow(theta,19.0)
    v40_2 = a7+torch.pow(theta, 19.0)
    ...
    v2_3 = 12.0*torch.pow(theta,11.0)+a8
    v4_3 = one/(a8+torch.pow(theta,11.0))
    v3_3 = v4_3*v4_3
    term5 = v1_3*v2_3*v3_3

    # "v" section 4
    v1_4 = torch.pow(a10+beta,-3.0) + a11*beta
